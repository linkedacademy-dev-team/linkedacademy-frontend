              ...
              <div class="sm:flex sm:items-start">
                ...
                <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                  <DialogTitle
                    ...
                    >Editar grupo etnico</DialogTitle
                  >
                  <div class="mt-4">
                    <label>
                      <span class="text-sm font-semibold text-gray-700"
                        >Nombre del grupo etnico</span
                      >
                      <input
                        v-model="ethnicGroupData.name"
                        type="text"
...
const editEthnicGroup = async () => {
  try {
    await store.dispatch("UPDATE_ETHNIC_GROUP", {
      id: ethnicGroupData.value.id,
      name: ethnicGroupData.value.name,
    });
    close();
    toast.success("Grupo etnico editado correctamente");
  } catch (error) {
    toast.error("Error al editar el grupo etnico");
  }
};
...
const ethnicGroupData = ref();

watch(
  () => props.ethnicGroup,
  (value) => {
    ethnicGroupData.value = value;
  }
);
