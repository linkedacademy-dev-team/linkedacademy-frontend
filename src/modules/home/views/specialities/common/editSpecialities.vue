              ...
              <div class="sm:flex sm:items-start">
                ...
                <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                  <DialogTitle
                    ...
                    >Editar especialidad</DialogTitle
                  >
                  <div class="mt-4">
                    <label>
                      <span class="text-sm font-semibold text-gray-700"
                        >Nombre de especialidad</span
                      >
                      <input
                        v-model="specialitieData.name"
                        type="text"
...
import { PlusCircleIcon,PencilIcon, XMarkIcon } from "@heroicons/vue/24/outline";
...
const editSpecialitie = async () => {
  try {
    await store.dispatch("UPDATE_SPECIALITY", {
      id: specialitieData.value.id,
      name: specialitieData.value.name,
    });
    close();
...
const specialitieData = ref();

watch(
  () => props.specialitie,
  (value) => {
    specialitieData.value = value;
  }
);

const emit = defineEmits(["close-edit-specialitie"]);

const close = () => emit("close-edit-specialitie");
</script>
